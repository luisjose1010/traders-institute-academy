...
const studentNav = [
  ...
  { id: "courses", label: "My Courses", icon: GraduationCap, path: "/dashboard" },
...
const adminNav = [
  ...
  { id: "courses", label: "Courses", icon: GraduationCap, path: "/dashboard" },
  { id: "students", label: "Students", icon: Users, path: "/dashboard" },
  { id: "access", label: "Grant Access", icon: ShieldCheck, path: "/dashboard" },
];

export function DashboardLayout({ children, activeSection, onSection, title }: {
    ...
    <div style={{ minHeight: "100vh", background: "#080808", color: "#fff", fontFamily: "Inter, sans-serif", display: "flex" }}>
      ...
      <aside style={{ width: 240, background: "#0a0a0a", borderRight: "1px solid rgba(255,255,255,0.06)", display: "flex", flexDirection: "column" as const, position: "fixed" as const, top: 0, left: 0, bottom: 0, zIndex: 40, transform: sidebarOpen ? "translateX(0)" : "translateX(-100%)", transition: "transform 0.25s ease" }} className="md:translate-x-0">
        <div style={{ padding: "1.25rem 1.5rem", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <img src="/traders-logo.png" alt="TIA" style={{ height: 30, cursor: "pointer" }} onClick={() => navigate("/")} />
        ...
        <nav style={{ flex: 1, padding: "0.5rem 0.75rem", display: "flex", flexDirection: "column" as const, gap: 2 }}>
          {navItems.map(item => {
            ...
            return (
              <button key={item.id} onClick={() => { onSection(item.id); setSidebarOpen(false); navigate(item.path); }} style={{ display: "flex", alignItems: "center", gap: 10, padding: "0.65rem 0.9rem", borderRadius: 8, background: active ? accentBg : "transparent", border: active ? `1px solid ${accentBorder}` : "1px solid transparent", color: active ? accentColor : "#777", fontSize: "0.85rem", fontWeight: active ? 600 : 400, cursor: "pointer", textAlign: "left" as const, width: "100%", transition: "all 0.15s" }}>
        ...
        <div style={{ padding: "1rem 0.75rem", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "0.65rem 0.9rem", borderRadius: 8, background: "rgba(255,255,255,0.03)", marginBottom: 6 }}>
            <div style={{ width: 32, height: 32, borderRadius: "50%", background: isAdmin ? "linear-gradient(135deg, #e74c3c, #c0392b)" : "linear-gradient(135deg, #C9A84C, #8a6a20)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.7rem", fontWeight: 700, color: isAdmin ? "#fff" : "#000" }}>{user?.initials ?? "?"}</div>
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: "0.82rem", fontWeight: 600, color: "#ddd", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const }}>{user?.name ?? "User"}</div>
              <div style={{ fontSize: "0.6rem", color: accentColor, fontWeight: 700 }}>{user?.role?.toUpperCase()}</div>
            </div>
          </div>
      ...
      <div style={{ flex: 1, minWidth: 0 }} className="md:ml-[240px]">
        <header style={{ height: 60, borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", padding: "0 1.5rem", background: "rgba(8,8,8,0.95)", backdropFilter: "blur(12px)", position: "sticky" as const, top: 0, zIndex: 20, gap: 12 }}>
          ...
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10 }}>
            <NotificationBell />
            <button onClick={() => navigate("/")} style={{ background: "none", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, padding: "6px 10px", cursor: "pointer", color: "#888", fontSize: "0.75rem" }}>Home</button>
            {isAdmin && <span style={{ fontSize: "0.6rem", fontWeight: 700, color: "#e74c3c", background: "rgba(231,76,60,0.1)", border: "1px solid rgba(231,76,60,0.2)", padding: "2px 8px", borderRadius: 4 }}>ADMIN</span>}
          </div>
        </header>
